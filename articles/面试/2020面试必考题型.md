...
- 创建私有变量,可以通过外部调用闭包函数，从而在外可以访问到函数内部的变量
- 可以让已经结束的函数上下文对象继续保留在内存中，闭包保存了对这个变量的引用，所以不会被回收。
...
- 每隔一段时间浏览器会向服务器发送一个相同的http请求到server以获取到数据的相应。
- server端收到请求后不管是否有数据达到，都会直接去响应http请求，然后服务器响应完成后，就会关闭这个TCP连接。
...
通过$on(callback,data)就可以拿到对应的数据了。
