...
向下滚动的时候，歌手详情页面不是有一个背景图片嘛，做的优化就是向上拉取的时候，对背景图片有一个放大缩小的效果，也就是使用scale属性，优化了用户的体验。

```js
this.$refs.bgImage.style[transform] = `scale(${scale})`
```

还有就是向上向下滚动的时候，图片有一个高斯模糊的效果，拖动向下滑动越高，也就是模糊度越高👇，达到渐进增强效果，

```
this.$refs.filter.style['backdrop-filter'] = `blur(${blur}px)`
```

以上两点实现的效果就是在watch中监听scrollY值，用scrollY 除以图片的高度，完成一个百分比效果percent